...
@onready var active: bool = false #is the terminal pod active

#---SETTERS-----------------------

# Set the correct state of the object from game memory.
func _set_active() -> void:
	# Variable for finding terminal in the currently active room.
	var terminal
	
	# Find the terminal.
	for i in game_manager.COLORS.size():
		# Get the id of the currently active room.
		var room_id = game_manager.get_dungeon().current_room.id 
		
		# Find which terminal is in currently active room.
		if room_id == game_manager.get_dungeon().terminals[i].room_id:
			# Set terminal to fund one and stop searching.
			terminal = game_manager.get_dungeon().terminals[i]
			break
	
	# Set stance of the terminal to found one.
